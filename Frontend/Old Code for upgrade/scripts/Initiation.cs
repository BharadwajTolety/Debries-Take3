...
public class Initiation : MonoBehaviour {
	...
	void Awake(){
		...
		//C2- Run DrawMap()
		JSONstring=File.ReadAllText(Application.dataPath+"/Node_data_6.json");
    //  JSONstring=File.ReadAllText(Application.dataPath+"/RealData_nodes.json");
      itemData = JsonMapper.ToObject (JSONstring);
        
		/*JSON_Edges=File.ReadAllText(Application.dataPath+"/Edges.json");
		edgeNamesData = JsonMapper.ToObject (JSON_Edges);
		JSON_Contractors=File.ReadAllText(Application.dataPath+"/contractors.json");
		ContractorsData = JsonMapper.ToObject (JSON_Contractors);
*/
		unit_r=300;
		TransX = 30;
		TransY = -100;
