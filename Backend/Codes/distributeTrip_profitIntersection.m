function [Contractor, change_check, updated_profit_vec] = distributeTrip_profitIntersection(Contractor,nc, from,to,coll_debris, TimeMatrix,...
    updated_profit_vec,revenue_per_debris, change_check,gas_per_distance, depot, capacity, Contractor_original,surrounding)
...
% for i = 1:length(debris_roads)
    ...
    %Instead of the following line of code in distributeTrip_profit we pick
    %the new contractor by also considering the intersection - Also
    %updating surrounding at each change
    %[~, I] = sort(updated_profit_vec, 'ascend');
    
    %Add it to the contractor appears the most in its surroundings
    ...
    while new_nc_notfound
        %Get the first max surrounding contractor that adding from,to edge would
        ...
        if iter > length(ben) && new_nc_notfound
            ...
            %Get the max surrounding contractor 
            [ ~ , I2] = max(s);           
            nc_new = I2; %max surrounding
            B = ben(nc_new);
            cluster_add_vec = clust{nc_new};
            path_add = path{nc_new};
            ccc = collect{nc_new};
